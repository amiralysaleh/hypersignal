...
async function getTrackedWalletsWithCooldown() {
  const wallets = await readWallets();
  return wallets.filter(wallet => wallet.isActive);
}

async function updateWalletCooldowns(walletAddresses, coin) {
  const wallets = await readWallets();
...
async function sendTelegramMessage(signal, settings) {
