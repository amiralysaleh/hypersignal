...
async function main() {
  ...
  if (source.includes('class AutomationScheduler')) {
    const classStart = source.indexOf('class AutomationScheduler');
    const braceStart = source.indexOf('{', classStart);

    if (braceStart === -1) {
      throw new Error('Unable to locate AutomationScheduler class body in worker');
    }
    ...
    if (classEnd === -1) {
      throw new Error('Unable to determine end of AutomationScheduler class definition in worker');
    }

    const classSource = source.slice(classStart, classEnd + 1);
    const rewrittenClassSource = classSource.replace(/(?<!storage)\.setAlarm/g, '.storage.setAlarm');

    source = `${source.slice(0, classStart)}${rewrittenClassSource}${source.slice(classEnd + 1)}`;
  } else {
