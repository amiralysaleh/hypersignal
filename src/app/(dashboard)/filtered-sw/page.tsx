...
import type { SuccessPredictionOutput, CorrelationGroup } from '@/types/analytics';
import { Progress } from '@/components/ui/progress';

async function fetchPrediction(): Promise<SuccessPredictionOutput> {
  const response = await fetch('/api/analytics/predict-signal-success', { cache: 'no-store' });
  if (!response.ok) {
...
async function fetchCorrelations(): Promise<CorrelationGroup[]> {
  const response = await fetch('/api/analytics/wallet-correlations', { cache: 'no-store' });
  if (!response.ok) {
    throw new Error('Failed to fetch correlations');
  }
  const data = await response.json();
  return data.groups as CorrelationGroup[];
}
...
export default function FilteredSwPage() {
  ...
  const fetchData = React.useCallback(async () => {
    try {
      setLoading(true);
      const [probData, corrData] = await Promise.all([fetchPrediction(), fetchCorrelations()]);
      setSuccessProbability(probData.successProbability);
      setCorrelationGroups(corrData);
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-muted-foreground">Leverage AI to predict signal success and detect wallet correlations.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Bot className="w-6 h-6" /> Signal Success Prediction
            </CardTitle>
            ...
            {loading ? (
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-2 text-center">
                  Based on analysis of recent signal performance and market conditions.
                </p>
              </div>
            ) : (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Users className="w-6 h-6" /> Wallet Correlation
            </CardTitle>
                  ...
                  <div key={index} className="p-3 bg-muted rounded-lg">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <div className="text-sm text-muted-foreground">
                        <span className="font-bold">{group.wallets.length}</span> Wallets |{' '}
                        <span className="font-bold">{group.tradeCount}</span> Trades
                      </div>
                    </div>
