...
const AUTOMATION_ACTIVE_RUN_GRACE_MS = Math.max(
  15_000,
  Number(process.env.AUTOMATION_ACTIVE_RUN_GRACE_MS ?? 9 * 60_000)
);
const AUTOMATION_MIN_INTERVAL_MS = Math.max(
  5 * 60_000,
  Number(process.env.AUTOMATION_MIN_INTERVAL_MS ?? 9 * 60_000)
);
...
export async function POST() {
  ...
  const lastDetectionRunAt = workerState.lastDetectionRunAt
    ...
    : NaN;
  if (Number.isFinite(lastDetectionRunAt)) {
    const sinceLastRunMs = startedAt - lastDetectionRunAt;
