...
import { getSignals } from './signal-service';
import { log } from './log-service';
import { z } from 'zod';
import type { SuccessPredictionOutput, CorrelationGroup, WalletCorrelationsOutput } from '@/types/analytics';

const SuccessPredictionOutputSchema = z.object({
  successProbability: z
    .number()
    .describe('A probability score from 0 to 100 representing the likelihood of the next signal hitting Take Profit.'),
});
  ...
  async () => {
    try {
      ...
      if (recentSignals.length < 10) {
        return { successProbability: 50.0 } satisfies SuccessPredictionOutput;
      }
      ...
      return output as SuccessPredictionOutput;
    } catch (error: any) {
      await log({ level: 'ERROR', message: 'Failed to execute predictSignalSuccess flow', context: { error: error.message } });
      return { successProbability: 50.0 } satisfies SuccessPredictionOutput;
    }
  }
);
...
export async function predictSignalSuccess(): Promise<SuccessPredictionOutput> {
  return predictSignalSuccessFlow({});
}
  ...
  async () => {
    try {
      ...
      if (recentSignals.length < 5) {
        return { groups: [] } satisfies WalletCorrelationsOutput;
      }
      ...
      const prompt = `
            ...
            Signal Data:
            ${recentSignals
              .map((s) => `- Wallets: [${s.contributingWalletAddresses.join(', ')}], Coin: ${s.pair}`)
              .join('\n')}
      ...
      const llmResponse = await ai.generate({
        prompt,
        output: { schema: WalletCorrelationsOutputSchema },
      ...
      const typedOutput = output as WalletCorrelationsOutput;
      typedOutput.groups.sort((a, b) => {
      ...
      await log({ level: 'ERROR', message: 'Failed to execute getWalletCorrelations flow', context: { error: error.message } });
      return { groups: [] } satisfies WalletCorrelationsOutput;
    }
  }
);
...
export async function getWalletCorrelations(): Promise<WalletCorrelationsOutput> {
  return getWalletCorrelationsFlow({});
}
